...
router.post('/login', async (req, res) => {
  try {
    ...
    // Use either email or username as the login input
    ...
    // Find user by either normalized email or username (case-insensitive)
    const user = await User.findOne({
      $or: [
        { email: { $regex: `^${loginInput}$`, $options: 'i' } },
        { username: { $regex: `^${loginInput}$`, $options: 'i' } }
      ]
    });
    if (!user) {
      console.log('[LOGIN] User not found', { loginInput });
    ...
    if (!isMatch) {
      console.log('[LOGIN] Password mismatch', { loginInput });
      res.status(401).json({ message: 'Invalid email/username or password' });
...
router.post('/request-password-reset', async (req, res) => {
  try {
    ...
    // Generate secure token and OTP for the reset
    const token = crypto.randomBytes(32).toString('hex');
    const expiry = Date.now() + 60 * 60 * 1000; // 1 hour
    const otp = brevoOtpService.generateOTP();

    // Save reset tokens to user
...
// Legacy OTP verification route for backwards compatibility
...
router.post('/verify-email-otp', async (req, res) => {
  try {
    ...
    const referralCode = registrationData.referralCode || await generateUniqueReferralCode();
    ...
    const newUser = new User({
      ...
      password: registrationData.password,
      registrationIP: registrationData.registrationIP || '',
      referralCode, // Add the generated or provided referral code
      isEmailVerified: true,
      wallets: {
        btc: { address: btcAddress, privateKey: btcPrivateKey, mnemonic: btcMnemonic },
        eth: { address: ethAddress, privateKey: ethPrivateKey, mnemonic: ethMnemonic },
