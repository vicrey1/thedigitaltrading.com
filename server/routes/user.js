...
// Get user's withdrawals (withdrawal history)
router.get('/withdrawals', auth, async (req, res) => {
  try {
    ...
    // Helper to resolve billing wallet address from user's wallets
    ...
    // Map to client-friendly format and include billingWalletAddress
    const mapped = withdrawals.map(w => {
      const walletKey = getWalletKey(w.currency, w.network);
    ...
    console.error('[USER WITHDRAWALS] Error fetching withdrawals:', err);
