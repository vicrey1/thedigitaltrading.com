...
async function getPortfolioData(userId) {
  ...
  // Subtract all pending withdrawals from available balance
  const pendingWithdrawals = await Withdrawal.find({
    userId: userId,
    status: { $in: ['pending_billing', 'pending', 'processing'] }
  });
  const totalPendingWithdrawals = pendingWithdrawals.reduce((sum, w) => sum + w.amount, 0);
  // Calculate availableBalance: depositBalance - totalInvested + totalConfirmedRoi + netAdminAdjustments - pending withdrawals
  const availableBalance = depositBalance - totalInvested + totalConfirmedRoi + netAdminAdjustments - totalPendingWithdrawals;
