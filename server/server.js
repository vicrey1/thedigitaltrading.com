...
// --- STATIC FILE SERVING FOR CAR IMAGES ---
// This must be before any API routes so both frontend and backend can access images
// Serve car images statically (robust, works in dev and prod)
// Serve car images statically (robust, works in dev and prod)
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // Allow hostname suffix matches (useful if origins in env omit subdomain prefixes)
    try {
      const originHost = origin.replace(/^https?:\/\//, '').replace(/:\d+$/, '');
      const allowed = corsOrigins.some(allowedOrigin => {
        const allowedHost = allowedOrigin.replace(/^https?:\/\//, '').replace(/:\d+$/, '');
        // exact match or suffix match (e.g., allowedOrigin 'thedigitaltrading.com' matches 'app.thedigitaltrading.com')
        return originHost === allowedHost || originHost.endsWith('.' + allowedHost) || allowedHost === originHost;
      });

      if (allowed) return callback(null, true);
    } catch (e) {
      console.warn('[CORS] origin parsing failed', e && e.message);
    }
...
// KYC uploads are served through authenticated route in /routes/uploads.js
