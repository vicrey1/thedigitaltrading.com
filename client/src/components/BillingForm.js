...
import { payBillingFee, getWithdrawalBillingStatus } from '../services/withdrawalAPI';
...
const BillingForm = ({ withdrawalData, onClose, onSuccess }) => {
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      // Call payBillingFee without PIN
      const response = await payBillingFee(withdrawalData.withdrawal.id);
      setSuccess(response.message || 'Payment confirmed');
  ...
  // Map USDT/USDC billing addresses to use deposit chain addresses
  const getDepositChainFromToken = (currency, network) => {
    if (network === 'ERC20') return { chain: 'eth', name: 'Ethereum (ERC20)' };
    if (network === 'TRC20') return { chain: 'tron', name: 'Tron (TRC20)' };
    return { chain: currency.toLowerCase(), name: network };
  };

  const depositChain = getDepositChainFromToken(cryptoDetails?.currency, cryptoDetails?.network);
  const effectiveBillingAddress = withdrawalData.depositWalletAddress || billingWalletAddress;
        ...
        <div className="billing-form-content">
          ...
          <div className="withdrawal-summary">
           ...
           {billingStatus?.status === 'pending_billing' ? 'Awaiting Fee Payment' : 
            billingStatus?.status === 'pending' ? 'Withdrawal Pending' :
          ...
          {billingWalletAddress && (
            <div className="payment-section">
              ...
              <div className="wallet-address-container">
                <label>Wallet Address ({depositChain.name}):</label>
                <div className="wallet-address">
                  ...
                  <button 
                    className="copy-btn" 
                    onClick={() => copyToClipboard(effectiveBillingAddress)}
          ...
          {billingStatus?.billingPaid && (
            <div className="payment-confirmed">
              <p>✅ Payment confirmed! Your withdrawal is now Withdrawal Pending.</p>
