...
const WithdrawalHistory = ({ withdrawals, onPendingBillingClick }) => {
  ...
  // Handler for clicking pending billing withdrawals
  const handleWithdrawalClick = (withdrawal) => {
    if (withdrawal.status === 'pending_billing' && onPendingBillingClick) {
        ...
        <div className="space-y-4">
          {list.map((withdrawal) => (
            <div 
              ...
              className={`flex justify-between items-start p-3 border-b border-gray-800 ${
                withdrawal.status === 'pending_billing' ? 'cursor-pointer hover:bg-gray-800' : ''
              }`}
