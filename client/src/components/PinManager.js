...
const PinManager = ({ onPinSet, showTitle = true }) => {
  ...
  const checkCurrentPinStatus = async () => {
    try {
      ...
      setError('');
      console.log('Checking current PIN status...');
      
      const response = await checkPinStatus();
      console.log('PIN status check result:', response);
      
      setHasPinSet(response.hasPinSet);
      
      // If PIN is already set, show informative message
      if (response.hasPinSet) {
        setMessage('Withdrawal PIN is already set. You can update it below.');
      }
    } catch (err) {
      console.error('Error checking PIN status:', err);
      setError('Failed to check PIN status. Please try refreshing the page.');
      setHasPinSet(false);
    } finally {
  ...
  const validatePin = (pinValue) => {
    ...
    // Removed weak PIN check to allow sequential numbers
    return null;
  };
      ...
      <div className="pin-form">
        ...
        <div className="pin-strength-info">
          ...
          <ul>
            ...
            {/* Removed sequential number restriction */}
