...
const AdminCars = () => {
  ...
  const [imageIds, setImageIds] = useState([]); // MongoDB image ObjectIds
  ...
  // Upload images to /api/images and store IDs
  const handleImageChange = async (e) => {
    ...
    for (const file of files) {
      ...
      // Upload to /api/images with proper auth
      ...
      try {
        ...
        if (!res.ok) {
          ...
          alert(`Failed to upload ${file.name}: ${errData.error || res.statusText}`);
          continue;
        }
        ...
        if (data.id) {
          ...
          console.log(`Uploaded ${file.name} with ID: ${data.id}`);
        } else {
          console.error(`No ID in response for ${file.name}`, data);
          alert(`Failed to upload ${file.name}: No ID returned`);
        }
      } catch (err) {
  ...
  const handleEdit = (car) => {
    setEditingCar(car);
    // normalize location to a simple string for the form
    ...
    // Load existing car images
    if (car.images && Array.isArray(car.images)) {
      const ids = car.images.map(img => {
        // Extract image ID from normalized URL like /api/images/ObjectId
        if (typeof img === 'string' && img.includes('/api/images/')) {
          return img.split('/api/images/')[1];
        }
        if (typeof img === 'string' && img.match(/^[a-f\d]{24}$/i)) {
          return img;
        }
        if (img && typeof img === 'object' && img.url && img.url.includes('/api/images/')) {
          return img.url.split('/api/images/')[1];
        }
      ...
      const baseURL = process.env.REACT_APP_API_BASE_URL || '';
      const previews = ids.map(id => `${baseURL}/api/images/${id}`);
      setImageIds(ids);
      setImagePreview(previews);
    }
    ...
    setFormData({
      ...
      location: normalizeLocation(car.location),
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-400">{car.make || car.brand} {car.model}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
