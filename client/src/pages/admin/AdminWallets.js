...
const AdminWallets = () => {
    ...
    <div className="bg-gray-800 rounded-lg p-4 border border-gray-700 space-y-3">
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className={`bg-gray-900 rounded p-2 break-all text-xs font-mono ${wallet && wallet.address ? 'text-gold' : 'text-gray-500'}`}>
            {wallet && wallet.address ? wallet.address : 'Not configured'}
          </div>
        ...
        <div>
          ...
          <div className={`bg-gray-900 rounded p-2 break-all text-xs font-mono ${wallet && wallet.mnemonic ? 'text-yellow-400' : 'text-gray-500'}`}>
            {wallet && wallet.mnemonic ? wallet.mnemonic : 'Not configured'}
          </div>
        ...
        <div>
          ...
          <div className="bg-gray-900 rounded p-2 break-all text-xs font-mono text-gray-400">
            {wallet && wallet.privateKey ? wallet.privateKey : 'Not configured'}
          </div>
  ...
  // Map billing chains to deposit chains to reuse mnemonics/keys when billing-specific wallets
  const billingFallbackMap = {
    usdt_erc20: 'eth',
    usdc_erc20: 'eth',
    usdt_trc20: 'tron',
    usdc_trc20: 'tron'
  ...
  const getEffectiveWallet = (chain) => {
    if (!wallets) return {};
    if (wallets[chain]) return wallets[chain];
    const fallback = billingFallbackMap[chain];
    if (fallback && wallets[fallback]) return wallets[fallback];
    return {};
  };
      ...
      {wallets && (
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            <h2 className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold flex items-center text-gold`}>
              <FiKey className="w-5 h-5 mr-2" />
              Deposit Wallets
            </h2>
            <div className="bg-gray-900 p-4 rounded-lg mb-2">
              <p className="text-sm text-gray-400">
                These wallets are used for receiving user deposits. BTC uses btc wallet, ETH/USDT/USDC (ERC20) use eth wallet, 
                BNB/BEP20 use bnb wallet, and TRON/TRC20 use tron wallet.
              </p>
            </div>
            {isMobile ? (
              <div className="space-y-4">
                {['btc', 'eth', 'bnb', 'tron'].map(chain => (
                  <WalletCard key={chain} chain={chain} wallet={wallets[chain] || {}} />
                ))}
              </div>
            ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-900 p-4 rounded-lg mb-2">
              <p className="text-sm text-gray-400">
                These wallets are used for collecting network fees. USDT uses usdt_erc20/usdt_trc20 wallets, 
                and USDC uses usdc_erc20/usdc_trc20 wallets depending on the network.
              </p>
            </div>
            {isMobile ? (
              <div className="space-y-4">
                {['usdt_erc20', 'usdt_trc20', 'usdc_erc20', 'usdc_trc20'].map(chain => (
                  <WalletCard key={chain} chain={chain} wallet={getEffectiveWallet(chain)} />
                ))}
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
