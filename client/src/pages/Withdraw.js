...
import { submitWithdrawal, checkPinStatus, verifyWithdrawalPin } from '../services/withdrawalAPI';
...
const Withdraw = () => {
  ...
  const { kycStatus, refreshUserContext } = useUser();
  ...
  // Fetch user balances and wallets
  useEffect(() => {
  ...
  const handleConfirmWithdrawal = async () => {
    ...
    try {
      ...
      if (!token) {
        setPinError('Authentication required. Please log in again.');
      ...
      try {
        ...
        setPinError(err.response?.data?.msg || 'Invalid PIN');
        setIsSubmitting(false);
        return;
      }
      ...
      // Store withdrawal in state regardless of billing requirement
      if (!result || !result.success) {
        setPinError(result?.msg || 'Withdrawal failed. Please try again.');
        setIsSubmitting(false);
        return;
      }
      
      // Generate transaction ID or use the one from response
      const fakeId = result.withdrawal?._id || `WD-${Math.random().toString(36).substr(2, 10).toUpperCase()}`;
      setTransactionId(fakeId);
      
      // Refresh withdrawals list to include the new pending withdrawal
      const updatedWithdrawals = await getUserWithdrawals();
      setWithdrawals(Array.isArray(updatedWithdrawals) ? updatedWithdrawals : []);

      // Check if billing is required
      if (result.billingRequired && result.billingFee > 0) {
        // For ERC20/TRC20 tokens, get the deposit wallet address of the base chain
        let depositWalletAddress = null;
        if (result.cryptoCurrency === 'USDT' || result.cryptoCurrency === 'USDC') {
          const baseChain = result.network === 'ERC20' ? 'eth' : result.network === 'TRC20' ? 'tron' : null;
          if (baseChain && userWallets?.[baseChain]?.address) {
            depositWalletAddress = userWallets[baseChain].address;
          }
        }

        // Include deposit wallet address in withdrawal response
    ...
    <div className="max-w-4xl w-full mx-auto p-2 sm:p-4 overflow-auto">
          ...
          <div className="flex space-x-4">
            ...
            <button
              onClick={() => {
                ...
                const pendingBilling = Array.isArray(withdrawals) ? withdrawals.find(w => w.status === 'pending_billing') : null;
                if (pendingBilling) {
              ...
              className="flex-1 py-3 rounded-lg font-bold bg-gold text-black hover:bg-yellow-600 transition"
              disabled={!(Array.isArray(withdrawals) && withdrawals.some(w => w.status === 'pending_billing'))}
            >
              Pay Billing Fee
            </button>
          </div>
      ...
      <div className="mt-12">
        ...
        <WithdrawalHistory 
          ...
          onPendingBillingClick={(withdrawal) => {
            setWithdrawalResponse({
              withdrawal,
              billingFee: withdrawal.billingFee,
              requestedAmount: withdrawal.amount,
              billingWalletAddress: withdrawal.billingWalletAddress,
              cryptoDetails: {
                currency: withdrawal.currency,
                network: withdrawal.network
              },
              feeReason: 'A 20% billing fee is required for this withdrawal.'
            });
            setStep(4); // Show billing form
          }}
        />
      </div>
      ...
      {/* Billing Form Modal */}
        ...
        <BillingForm 
          ...
          onClose={() => {
            // If this was from clicking a history item, just close the form
            // If this was from a new withdrawal, go back to step 1
            const isFromHistory = withdrawals.some(w => 
              w.id === withdrawalResponse.withdrawal.id || 
              w._id === withdrawalResponse.withdrawal._id
            );
            if (!isFromHistory) {
              setStep(1); // Go back to withdrawal form for new withdrawals
            }
            setWithdrawalResponse(null);
          }}
        />
